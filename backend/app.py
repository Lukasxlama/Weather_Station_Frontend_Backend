...
def row_to_packet(row):
    return {
        ...
        "sensor_data": {
            ...
            "timestamp": row["timestamp"],  # oder eigener sensor_ts falls vorhanden
        }
    }
...
def latest():
    row = query_db("SELECT * FROM packets ORDER BY timestamp DESC, id DESC LIMIT 1", one=True)
    if not row:
        return jsonify({"error": "no data"})
    return jsonify(row_to_packet(row))
